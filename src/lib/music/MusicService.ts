...
export class MusicService {
  constructor(private provider: MusicProvider, private db: AlbumDatabase) {}
  ...
  async getFeedAlbums(amount = FEED_ALBUMS_AMOUNT): Promise<AlbumData[]> {
    const cachedFeedAlbums = await this.db.getFeaturedAlbums(amount);
    ...
    const initalizedAlbumsForFeed = await this.provider.getFeaturedAlbums(
      amount
    );

    const feedAlbums = await this.db.getFeaturedAlbums(amount);
